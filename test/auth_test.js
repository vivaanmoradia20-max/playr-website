...
const mods=["js/config.js","js/avatars.js","js/sports-data.js","js/sports-data-2.js","js/spcl-data.js","js/sports-app.js","js/sports-universe.js","js/spcl.js","js/events-data.js","js/events-app.js","js/events-app-2.js","js/challenges.js","js/auth.js","js/auth-ui.js"];
...
await t("JOIN opens auth landing (WELCOME TO PLAYR + both paths)", async()=>{ click(".join-btn-desktop"); if(!$("#authModal")) throw new Error("no modal"); const x=$("#authRoot").textContent; if(!x.includes("WELCOME TO PLAYR.")) throw new Error("no welcome"); if(!x.includes("SIGN IN")||!x.includes("CREATE ACCOUNT")) throw new Error("missing paths"); return "landing + SIGN IN + CREATE ACCOUNT ✓"; });
await t("landing → Sign In (WELCOME BACK) ↔ Create Account (new fields)", async()=>{ 
  [...$$("#authRoot .btn")].find(b=>b.textContent.trim()==="SIGN IN").click();
  if(!$("#authRoot").textContent.includes("WELCOME BACK")) throw new Error("no welcome back");
  [...$$("#authRoot .btn")].find(b=>b.textContent.trim()==="CREATE ACCOUNT").click();
  const x=$("#authRoot").textContent;
  if(!x.includes("CREATE YOUR PLAYR ACCOUNT")) throw new Error("heading");
  if(!$("#su-gender")) throw new Error("no gender field");
  if(!$("#su-sport")) throw new Error("no sport field");
  return "gender + primary-sport fields ✓"; });
await t("empty signup → clear validation errors", async()=>{ click(".a-submit"); const n=$$(".a-err").length; if(n<4) throw new Error(n+" errors"); return n+" field errors (Please complete this field.)"; });
await t("invalid email/username/password formats caught", async()=>{ $$(".a-err").forEach(e=>e.remove()); val("su-name","Test User"); val("su-user","bad name!"); val("su-email","nope"); val("su-pw","short"); val("su-pw2","other"); click(".a-submit"); const errs=$$(".a-err").map(e=>e.textContent); if(errs.length<3) throw new Error(errs.join("|")); return errs.length+" errors ✓"; });
await t("password mismatch caught (spec message)", async()=>{ $$(".a-err").forEach(e=>e.remove()); val("su-user","testuser"); val("su-email","t@playr.com"); val("su-pw","password1"); val("su-pw2","password2"); click(".a-submit"); const e=$$(".a-err").find(x=>/Passwords do not match/.test(x.textContent)); if(!e) throw new Error("spec string missing"); return "Passwords do not match. ✓"; });
await t("signup → onboarding step 1 (Welcome)", async()=>{ $$(".a-err").forEach(e=>e.remove()); val("su-name","Aarav Sharma"); val("su-user","aaravs"); val("su-email","aarav@playr.com"); val("su-pw","playr123"); val("su-pw2","playr123"); val("su-gender","female"); val("su-sport","running"); click(".a-submit"); await wait(250); if(!$("#onboardModal")) throw new Error("no onboarding"); const x=$("#obRoot").textContent; if(!x.includes("WELCOME TO PLAYR.")) throw new Error("no welcome"); return "5 steps listed ✓"; });
await t("gender + sport stored on account (avatar-ready)", async()=>{ if(!A.user) throw new Error("no user"); if(A.user.gender!=="female") throw new Error("gender: "+A.user.gender); if(!A.user.sports.includes("running")) throw new Error("sport not stored"); return "gender→profile ✓"; });
await t("step 2: WHAT DO YOU PLAY? + multi-select", async()=>{ UI.obNext(); const x=$("#obRoot").textContent; if(!x.includes("WHAT DO YOU PLAY?")) throw new Error("heading"); const n=$$("#obSports .ob-sport").length; if(n<15) throw new Error(n+" sports"); UI.obPick("cricket"); UI.obPick("mountaineering"); const on=$$("#obSports .ob-sport.on").length; if(on!==3) throw new Error("state: "+on+" (seeded running should be pre-selected)"); return n+" sports, 3 selected (running pre-seeded from signup) ✓"; });
